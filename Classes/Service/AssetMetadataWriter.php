...
final class AssetMetadataWriter
{
    ...
    public function write(int $fileUid, int $languageId, array $metadata): int
    {
        ...
        $metadataUid = $query->select('uid')->from('sys_file_metadata')->where($query->expr()->eq('file', $query->createNamedParameter($fileUid, ParameterType::INTEGER)), $query->expr()->eq('sys_language_uid', $query->createNamedParameter(0, ParameterType::INTEGER)))->executeQuery()->fetchOne();
        if (false === $metadataUid) {
            throw new \RuntimeException('TYPO3 has no metadata record for the selected file. Re-index the file and try again.');
        }
        if ($languageId > 0) {
